const chatInput = document.querySelector("#chatinput");
const askBtn = document.querySelector("#askbtn");
const container = document.querySelector(".container");
const themeBtn = document.querySelector("#themebtn");
const deleteChatHistoryBtn = document.querySelector("#deletechathistorybtn");
import config from './config.js';

let userText = null;
const API_KEY = config.API_KEY;

themeBtn.addEventListener("click", () => {
    // Toggle class of body
    document.body.classList.toggle("light-mode");
    saveData() // To save the theme color after every toggle

    // To change the icon of themeBtn
    themeBtn.innerText = document.body.classList.contains("light-mode") ? "dark_mode" : "light_mode";
})

// To create chatDiv element to show user text as chat
const createElement = (html, className) => {
    const chatDiv = document.createElement("div");
    chatDiv.classList.add("chat", className);
    chatDiv.innerHTML = html;
    return chatDiv; // Return the chatDiv element
}

// Set up POST request and ChatGpt answer
const getChatGptAnswer = async (incomingChatDiv) => {
    const API_URL = `https://api.openai.com/v1/completions`;

    const paragraph = document.createElement("p");

    // Data & properties for the API request
    const requestOptions = {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${API_KEY}`
        },
        body: JSON.stringify({
            model: "gpt-3.5-turbo-instruct",
            prompt: userText,
            max_tokens: 2048,
            temperature: 0.2,
            n: 1,
            stop: null
        })
    }

    // Fetch data from api
    try {
        const res = await fetch(API_URL, requestOptions);
        answer = await res.json();
        paragraph.textContent = answer.choices[0].text.trim();
    } catch (error) {
        paragraph.classList.add("oops");
        paragraph.textContent = "Oops! Don't refresh your browser while retrieving the chat response. Please try again";
        console.log("Error is : ", error);
    }

    // set chat responces
    incomingChatDiv.querySelector(".typing").remove();
    incomingChatDiv.querySelector(".details").appendChild(paragraph);

    container.scrollTo(0, container.scrollHeight); // To scroll up to container's scrollHeight

    saveData(); // To save chat history on localStorage
}

// To copy text of chat element
const copyText = (copyBtn) => {
    const textElement = copyBtn.parentElement.querySelector("p");
    navigator.clipboard.writeText(textElement.textContent);
    copyBtn.textContent = "done";
    setTimeout(() => copyBtn.textContent = "content_copy", 500);
}

// To show typing view at the time of incoming message
const typingView = () => {
    const htmlContent = `<div class="content">
                             <div class="details">
                                 <img src="https://upload.wikimedia.org/wikipedia/commons/1/13/ChatGPT-Logo.png" alt="">
                                 <div class="typing">
                                     <div class="dot" style="--delay: 0.2s"></div>
                                     <div class="dot" style="--delay: 0.3s"></div>
                                     <div class="dot" style="--delay: 0.4s"></div>
                                 </div>
                             </div>
                             <span onclick="copyText(this)" class="material-symbols-rounded">content_copy</span>
                         </div>`;

    const incomingChatDiv = createElement(htmlContent, "incoming");
    container.appendChild(incomingChatDiv);

    container.scrollTo(0, container.scrollHeight); // To scroll up to container's scrollHeight

    getChatGptAnswer(incomingChatDiv);
}

const handleOutgoingChat = () => {
    userText = chatInput.value.trim();  // Get chatInput and remove extra spaces
    if (!userText) { return; }

    const htmlContent = `<div class="content">
                             <div class="details">
                                 <img src="data:image/jpeg;base64," alt="">
                                 <p></p>
                             </div>
                             <span onclick="copyText(this)" class="material-symbols-rounded">content_copy</span>
                         </div>`;

    const outgoingChatDiv = createElement(htmlContent, "outgoing");
    outgoingChatDiv.querySelector("p").textContent = userText;
    container.appendChild(outgoingChatDiv);

    document.querySelector(".defaultview")?.remove(); // To remove default page view when ask to ChatGpt

    container.scrollTo(0, container.scrollHeight); // To scroll up to container's scrollHeight

    saveData(); // To save chat history on localStorage
    chatInput.value = ""; // To clean chatInput after askBtn

    setTimeout(typingView, 500);
}

// To ask to ChatGpt
askBtn.addEventListener("click", handleOutgoingChat);

// To ask to ChatGpt by enter button
chatInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
        e.preventDefault();
        handleOutgoingChat();
    }
})

// To delete chat history from localStorage
deleteChatHistoryBtn.addEventListener("click", () => {
    if (confirm("Are you sure to delete chats history ?")) {
        localStorage.removeItem("chats");

        loadData();
    }
})

// To save chat history on localStorage
function saveData() {
    localStorage.setItem("chats", container.innerHTML);

    // To save theme color on localStorage
    localStorage.setItem('themecolor', document.body.classList);
}

// To load chat history from localStorage
function loadData() {
    // To show default view on page when chats not present
    const defaultView = `<h1 class="defaultview">ChatGpt gpt-5.X-turbo</h1>`;

    container.innerHTML = localStorage.getItem("chats") || defaultView; // To show default view on page when chats not present

    // To load theme color from localStorage
    document.body.classList = localStorage.getItem("themecolor");


    // To scroll when load data
    container.scrollTo(0, container.scrollHeight); // To scroll up to container's scrollHeight
}

loadData();
